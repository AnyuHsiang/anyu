...
function anyutv_post_categories() {
	...
	if ( $categories_list && anyutv_categorized_blog() ) {
		printf( '<span class="entry-meta-item cat-links"><i class="fa fa-folder-open"></i> ' . esc_html__( 'Posted in %1$s', 'anyutv' ) . '</span>', $categories_list ); // WPCS: XSS OK.
	}
...
function anyutv_post_tags() {
	...
	if ( $tags_list ) {
		printf( '<span class="entry-meta-item tags-links"><i class="fa fa-tags"></i> ' . esc_html__( 'Tagged %1$s', 'anyutv' ) . '</span>', $tags_list ); // WPCS: XSS OK.
	}
 ...
 * @return bool
 */
function anyutv_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'anyutv_categories' ) ) ) {
		...
		set_transient( 'anyutv_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so anyutv_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so anyutv_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in anyutv_categorized_blog.
 */
function anyutv_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'anyutv_categories' );
}
add_action( 'edit_category', 'anyutv_category_transient_flusher' );
 ...
 */
function anyutv_post_thumbnail( $is_linked = true ) {
 ...
 */
function anyutv_read_more() {

	if ( post_password_required() ) {
		return;
	}

	$is_enabled = anyutv_get_option( 'blog_more', true );
